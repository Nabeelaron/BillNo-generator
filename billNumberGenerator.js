...
let billNos = {};
let billNoPath = "./storeBillNo/billNo.json"
let clashPath = "./storeBillNo/clash.json"
...
function storeRandomBillNo(value) {
    ...
    if (results[0] && results[1] && results[2]) //found duplicate
    {
        //request for another
        // console.log("clash!!")
    ...
    } else if (results[0] && results[1]) //code and digit2 exists but not digit3
    {
...
function findBillNo(code, digit2, digit3) {
    ...
    if (billNoCode.includes(code)) {
        if ((Object.keys(billNos[code])).includes(digit2[0])) {
            if (((billNos[code])[digit2[0]]).includes(digit3[0]))
